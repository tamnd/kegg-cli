...
describes what users get. Pick a format with `-o`, or let kegg choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
kegg <command> -o table   # aligned columns for reading
kegg <command> -o jsonl   # one JSON object per line, for piping
kegg <command> -o json    # a single JSON array
kegg <command> -o csv     # spreadsheet friendly
kegg <command> -o tsv     # tab-separated
kegg <command> -o url     # just the URL column
kegg <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
kegg <command> --fields id,title,url
```
...
```bash
kegg <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
kegg <command>            # a table, because this is a terminal
kegg <command> | wc -l    # JSONL, because this is a pipe
```
